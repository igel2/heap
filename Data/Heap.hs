...
-- be less or equal the minima of both 'Heap' parameters. /The
...
-- |
-- /O(n)/. Creates a 'Heap' from an ascending list.
...
check Empty = True
check (Tree r x left right) = let
		leftRank  = rank left
		rightRank = rank right
	in (isEmpty left || findMin left >= x)
